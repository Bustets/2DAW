...
function pinto_tablero(){  // copio el array a los inputs
    for(var i=0; i<3 ; i++){
        for(var j=0; j<3 ; j++){  // recorro la matriz
            ...
            var existe_imagen = document.getElementById(id_casilla).parentNode.getElementsByTagName('img').length;
            if (existe_imagen) {
                if(tablero[i][j]=="o"){
                document.getElementById(id_casilla).nextSibling.src="o.png"
                }
                else if(tablero[i][j]=="x") {
                    document.getElementById(id_casilla).nextSibling.src="x.png"
                }
                else{
                    document.getElementById(id_casilla).nextSibling.src="_.png"
                }
            }else{
                ...
                if(tablero[i][j]=="o"){
                    ...
                    nodo.setAttribute("src", "x.png");
                }
                else{
                    nodo.setAttribute("src", "_.png");
                }
                document.getElementById(id_casilla).parentNode.appendChild(nodo);
            }
        }
    }
}
...
function compruebo_filas_columnas_diagonales(jugador){  // compruebo si se gana o empata
       ...
       else if((tablero[1][0]==tablero[1][1])&&(tablero[1][1]==tablero[1][2])&&(tablero[1][2]==jugador)){
            alert (" Ha ganado "+jugador);
            location.reload();
        }
       else if((tablero[2][0]==tablero[2][1])&&(tablero[2][1]==tablero[2][2])&&(tablero[2][2]==jugador)){
            alert (" Ha ganado "+jugador);
            location.reload();
        }
        else if((tablero[0][0]==tablero[1][1])&&(tablero[1][1]==tablero[2][2])&&(tablero[2][2]==jugador)){ //diagonales
            alert (" Ha ganado "+jugador);
            location.reload();
        }
       else if((tablero[0][2]==tablero[1][1])&&(tablero[1][1]==tablero[2][0])&&(tablero[2][0]==jugador)){
            alert (" Ha ganado "+jugador);
            location.reload();
        }
        else if((tablero[0][0]==tablero[1][0])&&(tablero[1][0]==tablero[2][0])&&(tablero[2][0]==jugador)){ //columnas
            alert (" Ha ganado "+jugador);
            location.reload();
        }
        else if((tablero[0][1]==tablero[1][1])&&(tablero[1][1]==tablero[2][1])&&(tablero[2][1]==jugador)){
            alert (" Ha ganado "+jugador);
            location.reload();
        }
        else  if((tablero[0][2]==tablero[1][2])&&(tablero[1][2]==tablero[2][2])&&(tablero[2][2]==jugador)){
            alert (" Ha ganado "+jugador);
            location.reload();
        }
        else if(fin_partida==4){   // si la maquina lleva 4 turnos, se acaba en empate
            alert("EMPATE!!!!");
            location.reload();
        }
}
